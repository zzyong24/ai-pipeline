...
当前已完成：
- 4 个 SubGraph：`research` / `transcribe` / `write_book` / `routing`
- 1 个 Pipeline：`01-video-md`（视频→书稿，含 Langfuse 埋点 + 内容路由落库）
- 测试体系：`tests/unit/` + `tests/e2e/`（34 项，全部通过）

Roadmap（活跃任务见 `.aies/tasks/`）。
...
5. `.aies/spec/testing.md` — 测试规范（新增任务时必读）
6. `.ai/index.md` — 项目地图（SubGraph 清单 / Pipeline 清单 / 文件导航）
7. `.ai/review-checklist.md` — 审查清单
...
• acceptance.md 已准备：[是 / 否]
```

### Phase 2：执行

按清单读取参考文件 → 先写测试（参照 acceptance.md）→ 再写实现。
...
2. 测试验收（参照 acceptance.md）：
   ...
   - [ ] E2E Happy Path 通过
   - [ ] acceptance.md P0 场景打勾
3. 索引更新：[已更新 .ai/index.md / 无需]
4. 建议 commit message：`type(scope): 描述 [ai-assisted]`
5. 会话日志：python3 .aies/scripts/session.py add --title "..." --summary "..."
6. Spec 缺口：[有/无新约定需沉淀]
7. 技术债更新：[是否需要更新 .ai/known-issues.md]
```
...
| 用户说... | AI 应该读 | 参考模板 |
|----------|---------|---------|
| 新增 SubGraph | `architecture.md` + `testing.md` + `context-guide.md` 场景 1 | `subgraphs/research_subgraph/` |
| 新增 Pipeline | `architecture.md` + `context-guide.md` 场景 2 | `01-video-md/main_graph.py` |
| 修改 fan-out | `code-style.md` 反模式 + `context-guide.md` 场景 4 | `route_dispatcher` |
| 可观测问题 | `subgraphs/shared/observability.py` + `observability/langfuse_setup.md` | — |
| Bug 修复 | `.ai/known-issues.md` + 出问题文件 + 调用方 | — |
| 写测试 | `testing.md` + 对应 `acceptance.md` | `tests/unit/` + `tests/e2e/` |
...
.venv/bin/python3 -m pytest tests/ -v        # 全量（34 项）
.venv/bin/python3 -m pytest tests/unit/ -v   # 单元测试
.venv/bin/python3 -m pytest tests/e2e/ -v    # E2E 测试
```
...
# ↑ 自动生成 task.json + prd.md + acceptance.md 骨架
...
- ❌ observability 相关代码失败时影响 Pipeline 主流程（必须 fail-safe）
